...
- **婚禮規劃頁**（planner/）：
  - 勾選進度同步 GAS、離線 localStorage 備份、婚期自動同步
  - 日期區間編輯（✏️ inline + GAS 儲存 + 重置）
  - 狀態徽章（🔴🟡🟢）依自訂日期計算（customDates 優先）
  - Timeline 頁動態重建（從 CL 陣列按年月分組，婚期月標示 🎊）
  - Checklist 類別內依截止日升序排列（無日期項目排最後）
...
`Preparation item/` 資料夾放著婚禮規劃工具，作為**費用資料來源與設計參考**。此資料夾有版本控制但不部署到 GitHub Pages。
...
**工作表 1：甘特圖**
- 43 個待辦事項，從確認規劃到婚後事項
- C2 格輸入婚期（預設 2026/12/06），修改後圖列與所有 Checklist 到期日自動連動
- 色碼：藍=男方、粉=女方、紫=雙方、綠=已完成、紅=婚禮當天、黃欄=今日、粉欄=婚禮月
- 隱藏輔助欄存放 EDATE 起訖日公式，請勿刪除

**工作表 2：完整 Checklist（52 項）**
- 欄位：完成勾選（☐/☑）、項目說明、負責方、Deadline、到期日（連動 C2）、狀態（🔴🟡🟢）、台灣習俗備註、預算參考
- 涵蓋求婚→確認規劃→預訂服務→法律文件→男女方準備→拍攝→婚禮用品→婚禮當天→婚後事項

**工作表 3：男方清單（廖千儀，18 項）**
- 提親家長面談、確認婚期（農民曆）
- 聘金（大聘）、聘禮六禮、禮餅訂製
- 結婚對戒、婚房布置
- 迎娶車隊、新娘捧花、開門紅包、壓桌錢、祭祖供品
- 西裝/禮服（建議兩套：迎娶西裝＋晚宴服）
- 婚宴場地主導、男方賓客名單、主持人確認
...
- 婚紗挑選、試穿、試妝
- 婚禮配件（耳環、項鍊）
- 嫁妝：新棉被（喜被）、臉盆組（放四喜：桂圓/紅棗/花生/瓜子）、子孫桶
- 過門扇（出門丟一把、車內備一把）
- 出嫁儀式流程（梳妝→等待→開門→拜別父母→哭嫁→丟扇→出發）
- 歸寧準備（婚後第 3 天帶雞腿回娘家，當天返回不過夜）
- 婚後姓氏更改、銀行/保險更名

**工作表 5：使用說明**
- Excel 操作：修改婚期、打勾完成、逾期警示、甘特圖色碼、新增項目方式
- 台灣習俗說明：聘金、六禮/十二禮、囍餅、嫁妝、臉盆四喜、子孫桶、迎娶流程、過門扇、哭嫁、歸寧、農民曆擇日
- Q&A 7 則（婚期選擇、場地預訂時機、新秘預訂、聘禮準備、結婚登記、甘特圖排錯等）
- 版本紀錄：v1.0（2026/05 初始）、v2.0（2026/05 甘特圖連動＋使用說明）

### `wedding_planner.html`（設計參考）
互動式 HTML 婚禮規劃頁（827 行），功能：
...
### 自動維護規則（必須遵守）
每次對話中只要做了以下任何一件事，**必須在同一次對話結束前主動更新 CLAUDE.md**，不需等用戶提醒：
...
| 新增、刪除、重新命名檔案 | `檔案結構` |
| 更動費用類別 | `費用類別` |
| 更動技術架構（API、部署方式等） | `技術架構` |
| 婚禮規劃有新進展（婚期、場地等） | `基本資訊` 或相關區塊 |
| 更新 Preparation item 的規劃文件 | `Preparation item 資料夾` |
| 版本號升級 | `目前版本` 與 `版本紀錄` |
...
| 2026/05/27 | v1.1.0 Planner 勾選狀態同步 GAS（新增 Checklist.gs） |
| 2026/05/27 | v1.2.0 Planner 日期區間編輯（inline ✏️ + GAS 儲存 + 重置預設） |
| 2026/05/27 | v1.3.0 狀態徽章同步自訂日期、Timeline 動態重建、類別內依日期排序 |
